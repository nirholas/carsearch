...
console.log('        makes in select:', await page.locator('#h-make option').count(),
  '| tabs:', await page.locator('.tab').count(),
  '| panels:', await page.locator('.panel').count(),
  '| hero photo:', await page.evaluate(() => Boolean((document.querySelector('#hero-bg') as HTMLElement)?.style.backgroundImage)));

// Exercise the structured form first, which is the new path.
await page.selectOption('#h-make', 'Porsche');
await page.waitForTimeout(300);
await page.selectOption('#h-model', 'Macan');
await page.click('#adv-toggle');
await page.fill('#h-priceMax', '40000');
