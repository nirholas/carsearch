...
import { numberIn, priceOf, placeOf, placeFromAccount, type DdcVehicle } from '../src/sources/dealerdotcom.js';
...
test('a group listing gets its location from the rooftop that holds it', () => {
  // Group storefronts omit locationDistance entirely, so the first fifteen
  // groups wired stored 473 Macans with a null location until this join.
  assert.equal(
    placeFromAccount({ name: 'Acura of Honolulu', address: { city: 'Honolulu', state: 'HI' } }),
    'Acura of Honolulu (Honolulu, HI)',
  );
  // Partial records still say something useful rather than nothing.
  assert.equal(placeFromAccount({ address: { city: 'Novi', state: 'MI' } }), 'Novi, MI');
  assert.equal(placeFromAccount({ name: 'Porsche Novi' }), 'Porsche Novi');
  assert.equal(placeFromAccount(undefined), null);
  assert.equal(placeFromAccount({}), null);
});
