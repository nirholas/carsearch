...
export const SOURCES: Source[] = [
  ...
  s({ id: 'hemmings', name: 'Hemmings', homepage: 'https://www.hemmings.com', category: 'auction-enthusiast',
      status: 'planned', transport: 'tls', access: 'html', priceKinds: ['ask', 'bid'],
      notes: 'Recorded blocked on 2026-09-08 and REACHABLE on 2026-09-09, with no change at their end. The earlier test tried the bare chrome and firefox aliases and read two 403s as a wall. firefox133 answers 200 with 692KB and 51 prices, so the firefox alias resolves to a build old enough to be fingerprinted while a pinned recent one is not. That is why PROFILES now covers families rather than aliases. Access-solved, extraction-unsolved, and unlike the Autotrader case that phrase is earned: the page is 692KB of real inventory, not a block shell. Its only JSON-LD is Organization, WebSite and BreadcrumbList, so listings live in the markup and an adapter means real HTML parsing. RATE LIMITED HARD, measured over a timed sequence: two requests 20s apart both returned 200 and 51 prices, the third returned a Cloudflare interstitial, and roughly 90s of quiet cleared it again. So an adapter needs waitForTurn with a gap in the tens of seconds, not the default 3000ms, and must treat the interstitial as throttling rather than a block. Around 50 listings per page against that budget is why it is recorded as reachable but not yet wired.' }),
