...
export const SOURCES: Source[] = [
  ...
  s({ id: 'kengarff', name: 'Ken Garff', homepage: 'https://www.kengarff.com', category: 'dealer-group', countries: ['US'],
    status: 'live', transport: 'fetch', access: 'feed',
    notes: 'Wired 2026-09-12 through the Cars Commerce listings API that every Dealer Inspire storefront calls: POST websites-search.api.carscommerce.inc/api/v1/listings/<account>/search. A top-ten US group reporting 5,711 used cars across roughly seventy rooftops its own site treats as one inventory. Records are typed, so VIN, trim, odometer, drivetrain, engine, economy, the dealer address and the real vehicle detail URL are read rather than parsed. The account id and the x-api-key both live in a JavaScript bundle rather than the HTML, so they are learned from one browser page load and cached; a correctly shaped body without the header answers 401 "No API key found in request".' }),
  s({ id: 'westherr', name: 'West Herr', homepage: 'https://www.westherr.com', category: 'dealer-group', countries: ['US'],
    status: 'live', transport: 'fetch', access: 'feed',
    notes: 'Wired 2026-09-12 on the Cars Commerce platform, account 5394588, 2,669 used cars. Files its used stock under Used and Certified Used rather than Ken Garff\'s Pre-Owned, which is why the condition filter is read from each account instead of hardcoded: the first version returned zero here and read as a group with no used inventory.' }),
  s({ id: 'germain', name: 'Germain Motor Company', homepage: 'https://www.germaincars.com', category: 'dealer-group', countries: ['US'],
    status: 'live', transport: 'fetch', access: 'feed',
    notes: 'Wired 2026-09-12 on the Cars Commerce platform, account 5386274, 1,868 used cars across Ohio, Florida and Michigan.' }),
  s({ id: 'walser', name: 'Walser Automotive', homepage: 'https://www.walser.com', category: 'dealer-group', countries: ['US'],
    status: 'live', transport: 'fetch', access: 'feed',
    notes: 'Wired 2026-09-12 on the Cars Commerce platform, account 5386294. Minnesota and Kansas rooftops.' }),
  s({ id: 'kellyauto', name: 'Kelly Automotive', homepage: 'https://www.kellyauto.com', category: 'dealer-group', countries: ['US'],
    status: 'live', transport: 'fetch', access: 'feed',
    notes: 'Wired 2026-09-12 on the Cars Commerce platform, account 1317, 431 used cars in Massachusetts.' }),
  s({ id: 'bmwcamarillo', name: 'BMW of Camarillo', homepage: 'https://www.bmwofcamarillo.com', category: 'dealer-group', countries: ['US'],
    status: 'live', transport: 'fetch', access: 'feed',
    notes: 'Wired 2026-09-12 on the same Cars Commerce platform as Ken Garff, and reached with Ken Garff\'s key and no page load at all: the x-api-key is NOT account-scoped, so a rooftop costs an account id and nothing else. That is the point of wiring a platform rather than a site. Koons was tried first and dropped: its HTML carries a Dealer Inspire marker but no path on it ever calls the listings API, so it is on a different backend and an adapter over it would have returned nothing.' }),
  ...
  s({ id: 'herbchambers', name: 'Herb Chambers', homepage: 'https://www.herbchambers.com', status: 'live', transport: 'tls', access: 'payload', category: 'dealer-group', countries: ['US'],
    notes: 'Wired 2026-09-12, dealer.com widget API. 2,029 used cars. Answers to siteId asbtheherbchamberscompanies.' }),
  s({ id: 'jimellis', name: 'Jim Ellis Automotive', homepage: 'https://www.jimellis.com', status: 'live', transport: 'tls', access: 'payload', category: 'dealer-group', countries: ['US'],
    notes: 'Wired 2026-09-12, dealer.com widget API. 1,994 used cars. Answers to siteId jimellis.' }),
  s({ id: 'leithcars', name: 'Leith Automotive', homepage: 'https://www.leithcars.com', status: 'live', transport: 'tls', access: 'payload', category: 'dealer-group', countries: ['US'],
    notes: 'Wired 2026-09-12, dealer.com widget API. 1,846 used cars. Answers to siteId ddcleithgroupraleigh.' }),
  s({ id: 'garberauto', name: 'Garber Automotive', homepage: 'https://www.garberauto.com', status: 'live', transport: 'tls', access: 'payload', category: 'dealer-group', countries: ['US'],
    notes: 'Wired 2026-09-12, dealer.com widget API. 1,726 used cars. Answers to siteId garberautogroup.' }),
  s({ id: 'huffines', name: 'Huffines Auto Dealerships', homepage: 'https://www.huffines.net', status: 'live', transport: 'tls', access: 'payload', category: 'dealer-group', countries: ['US'],
    notes: 'Wired 2026-09-12, dealer.com widget API. 1,110 used cars. Answers to siteId huffinesautogroup.' }),
  s({ id: 'fermanauto', name: 'Ferman Automotive', homepage: 'https://www.fermanauto.com', status: 'live', transport: 'tls', access: 'payload', category: 'dealer-group', countries: ['US'],
    notes: 'Wired 2026-09-12, dealer.com widget API. 1,049 used cars. Answers to siteId ferman.' }),
  s({ id: 'jakesweeney', name: 'Jake Sweeney Automotive', homepage: 'https://www.jakesweeney.com', status: 'live', transport: 'tls', access: 'payload', category: 'dealer-group', countries: ['US'],
    notes: 'Wired 2026-09-12, dealer.com widget API. 912 used cars. Answers to siteId jakesweeney.' }),
  s({ id: 'hallauto', name: 'Hall Automotive', homepage: 'https://www.hallauto.com', status: 'live', transport: 'tls', access: 'payload', category: 'dealer-group', countries: ['US'],
    notes: 'Wired 2026-09-12, dealer.com widget API. 719 used cars. Answers to siteId hallautomotivevirginiabeach.' }),
  s({ id: 'mileone', name: 'MileOne Autogroup', homepage: 'https://www.mileone.com', status: 'live', transport: 'tls', access: 'payload', category: 'dealer-group', countries: ['US'],
    notes: 'Wired 2026-09-12, dealer.com widget API. 3,399 used cars. Answers to siteId mileoneautomotive.' }),
  s({ id: 'hertrichs', name: 'Hertrich Family of Dealerships', homepage: 'https://www.hertrichs.com', status: 'live', transport: 'tls', access: 'payload', category: 'dealer-group', countries: ['US'],
    notes: 'Wired 2026-09-12, dealer.com widget API. 999 used cars. Answers to siteId hertrichs.' }),
  s({ id: 'mcgeecars', name: 'McGee Auto Group', homepage: 'https://www.mcgeecars.com', status: 'live', transport: 'tls', access: 'payload', category: 'dealer-group', countries: ['US'],
    notes: 'Wired 2026-09-12, dealer.com widget API. 720 used cars. Answers to siteId mcgeemgmtcogroupwakefield.' }),
  s({ id: 'hendrick', name: 'Hendrick Automotive Group', homepage: 'https://www.hendrickcars.com', status: 'live', transport: 'tls', access: 'payload', category: 'dealer-group', countries: ['US'],
    notes: 'Wired 2026-09-12, dealer.com widget API. 11,719 used cars. siteId is hendrickautogroup, not the domain, and its first listing.config.id on the page is auto-exotic-used which returns 20 cars; auto-used returns all of them.' }),
  s({ id: 'sonicautomotive', name: 'Sonic Automotive', homepage: 'https://www.sonicautomotive.com', status: 'live', transport: 'tls', access: 'payload', category: 'dealer-group', countries: ['US'],
    notes: 'Wired 2026-09-12, dealer.com widget API. 10,541 used cars. Its first listing.config.id is auto-new, which answers with a healthy 24,074 that are new cars.' }),
  s({ id: 'suburbancollection', name: 'Suburban Collection', homepage: 'https://www.suburbancollection.com', status: 'live', transport: 'tls', access: 'payload', category: 'dealer-group', countries: ['US'],
    notes: 'Wired 2026-09-12, dealer.com widget API. 3,503 used cars. Answers to siteId lithiacollection.' }),
  s({ id: 'ourisman', name: 'Ourisman Automotive', homepage: 'https://www.ourisman.com', status: 'live', transport: 'tls', access: 'payload', category: 'dealer-group', countries: ['US'],
    notes: 'Wired 2026-09-12, dealer.com widget API. 3,327 used cars; the page-declared auto-used on-platform is a 1,706-car subset.' }),
  s({ id: 'fredbeans', name: 'Fred Beans Automotive', homepage: 'https://www.fredbeans.com', status: 'live', transport: 'tls', access: 'payload', category: 'dealer-group', countries: ['US'],
    notes: 'Wired 2026-09-12, dealer.com widget API. 2,572 used cars. Answers to siteId fredbeansdoylestown3.' }),
  s({ id: 'tomwood', name: 'Tom Wood Automotive', homepage: 'https://www.tomwood.com', status: 'live', transport: 'tls', access: 'payload', category: 'dealer-group', countries: ['US'],
    notes: 'Wired 2026-09-12, dealer.com widget API. 750 used cars.' }),
  s({ id: 'autofair', name: 'AutoFair', homepage: 'https://www.autofair.com', status: 'live', transport: 'tls', access: 'payload', category: 'dealer-group', countries: ['US'],
    notes: 'Wired 2026-09-12, dealer.com widget API. 520 used cars. Answers to siteId amsiautofair.' }),
  ...
  s({ id: 'jdmbuysell', name: 'JDMBUYSELL', homepage: 'https://www.jdmbuysell.com', category: 'marketplace', countries: ['US', 'CA', 'JP'],
    status: 'live', transport: 'tls', access: 'jsonld',
    notes: 'Wired 2026-09-12. The largest marketplace for 25-year-rule imports: 923 kei trucks from 57 sellers, plus Skylines, Supras and the rest. Ads are Product+Vehicle JSON-LD, 96 a page, ?page=N. No location in the ad nodes; the only address in the page is JDMBUYSELL itself in Burnaby, BC, so reading the first address puts every truck in Canada. Some sellers are still in Japan (MITSUI Co., Ltd., Uozu) and their prices exclude freight and customs. Odometers are KMT.' }),
  ...
  s({ id: 'fourbie', name: 'Fourbie Exchange', homepage: 'https://fourbieexchange.com', category: 'marketplace', countries: ['US'],
    status: 'live', transport: 'tls', access: 'html',
    notes: 'Wired 2026-09-12. A marketplace for imported 4x4s, vans and kei trucks where private sellers and small importers list side by side with a location on every card, which is what a buyer shopping one state needs and what importer shops cannot give. No API and no catalogue JSON, so the listing cards are read; the classes are named for what they hold (listingCard__price, listingCard__location, listingCard__title), a steadier hook than most markup. The kei category is a single page of about 34 cards with no pagination, and the site search is not usable for this: q=hijet returns 1,648 unrelated vehicles led by a Toyota Tacoma. Cards write mileage as 11K mi, which the shared parser read as eleven miles until the shorthand was expanded.' }),
  // Kei truck and mini truck specialists on commodity shop software.
  s({ id: 'keitrucksamerica', name: 'Kei Trucks America', homepage: 'https://www.kei-trucks.com', category: 'specialty', countries: ['US'],
    status: 'live', transport: 'tls', access: 'feed',
    notes: 'Wired 2026-09-12 through the Shopify catalogue every Shopify store publishes at /products.json. West Lafayette, Indiana. A small importer whose catalogue is its whole inventory, sold as trucks rather than as parts; a Deposit product that reserves a truck is in the same catalogue and is excluded, because it is not a truck for sale.' }),
  s({ id: 'minitrucksnet', name: 'Minitrucks.net', homepage: 'https://minitrucks.net', category: 'specialty', countries: ['US'],
    status: 'live', transport: 'tls', access: 'feed',
    notes: 'Wired 2026-09-12 through its Shopify /products.json. A Japan-based exporter with stock in Japan, the US and Australia: 311 products, 134 of them vehicles filed under product_type Vehicle, the rest parts. Its per-product Location_ tag overrides the shop location, because one truck listed there is sitting in Australia and a US buyer has to know that first. Odometers are quoted in kilometres in the description and are converted.' }),
  s({ id: 'jpmminitrucks', name: 'JPM Mini Trucks', homepage: 'https://jpmminitrucks.com', category: 'specialty', countries: ['US'],
    status: 'live', transport: 'tls', access: 'feed',
    notes: 'Wired 2026-09-12 through the public WooCommerce Store API at /wp-json/wc/store/v1/products. Miami, Florida. 383 products, about 150 of them vehicles. Most are Import-to-Order, a truck still in Japan bought on the buyer\'s behalf, which is a real offer at a real price and is kept with its location stated as Japan. Attributes give make, model, drivetrain, transmission and body style directly. Prices are in minor units (810000 with a unit of 2 is $8,100).' }),
  s({ id: 'minitruckimports', name: 'Mini Truck Imports', homepage: 'https://minitruckimports.com', category: 'specialty', countries: ['US'],
    status: 'live', transport: 'tls', access: 'feed',
    notes: 'Wired 2026-09-12 through the WooCommerce Store API. Lot in Louisiana. Its attributes are the most complete in the segment: stock number, year, make, model, transmission, options and the Japanese chassis number (S211P-0021618), which is kept out of the VIN column because it is not a 17-character VIN and would let dedupe match on a serial that is unique only within a model code.' }),
  s({ id: 'keitrucksmarket', name: 'Kei Trucks Market', homepage: 'https://keitrucksmarket.com', category: 'specialty', countries: ['US'],
    status: 'live', transport: 'tls', access: 'feed',
    notes: 'Wired 2026-09-12 through the WooCommerce Store API. Federal Way, Washington. 215 vehicles, and not only kei trucks: it also sells imported JDM cars such as the Toyota Crown and E46 BMWs, which are real stock and are kept, bounded by whatever make and model the search asked for.' }),
  s({ id: 'keitruckimporters', name: 'Kei Truck Importers', homepage: 'https://keitruckimporters.com', category: 'specialty', countries: ['US'],
    status: 'live', transport: 'tls', access: 'feed',
    notes: 'Wired 2026-09-12 through the WooCommerce Store API. A handful of trucks at a time and no published address, so its location is stated as not published rather than guessed.' }),
  ...
  s({ id: 'classiccars', name: 'ClassicCars.com', homepage: 'https://classiccars.com', status: 'live', transport: 'tls', access: 'jsonld', category: 'auction-enthusiast',
    notes: 'Wired 2026-09-10. Types its entries \'car\' in lower case, which schema.org does not define and only a case-insensitive type match finds. Offer urls are relative. No odometer in the payload.',
  }),
  ...
  s({ id: 'hagertymarketplace', name: 'Hagerty Marketplace', homepage: 'https://www.hagerty.com/marketplace', category: 'auction-enthusiast', priceKinds: ['sold', 'bid'],
      status: 'live', transport: 'fetch', access: 'feed',
      notes: 'Wired 2026-09-10. Its frontend searches Typesense at dora.production.collecting.com/multi_search with a scoped key published to every visitor. The long-standing 401 was never the key: a Typesense scoped key names the collections it may read and rejects any other as unauthorized, and the collection is production_listings, not the auctions that had been guessed. Recovering it needed the REQUEST body their frontend sends, not the response, because a search API is a POST and the collection, the queried fields and the filter syntax live only in the request. Every query must carry filter_by sites:=cars, or the same index returns number-plate and memorabilia lots alongside the cars. listingStage:sold reaches 26,244 completed sales with price, currency and the exact moment of sale, 4,241 of them Porsche, which is the scarcest input this index has.' }),
  s({ id: 'mecum', name: 'Mecum', homepage: 'https://www.mecum.com', category: 'auction-enthusiast', priceKinds: ['sold'],
    status: 'live', transport: 'fetch', access: 'feed',
    notes: 'Wired 2026-09-11 over the WPGraphQL endpoint at mecum.stellate.sh that their own site queries, found by capturing the request their frontend sends rather than only its response. An earlier sweep read the rendered page, found no year, price or sold key anywhere in the React payload, and correctly concluded the lots are client-fetched; this is where the client fetches them. Introspection is open, so the schema can be read rather than guessed. A Lot carries hammerPrice, vinSerial, odometer with its unit, engine, transmission, colour and runDates, which is the exact day the lot crossed the block. Two traps: the catalogue mixes memorabilia in and titles it like a car (a 2014 Porsche lighted sign sold for $960), so a lot must be filed under a make taxonomy AND survive the non-vehicle phrase list; and hideHammerPrice and hideSaleResult stay populated when set, so a withheld result must be skipped rather than read past.' }),
  s({ id: 'barrettjackson', name: 'Barrett-Jackson', homepage: 'https://www.barrett-jackson.com', category: 'auction-enthusiast', priceKinds: ['sold'],
    status: 'live', transport: 'fetch', access: 'payload',
    notes: 'Wired 2026-09-14 from the completed vehicle records embedded in the homepage React Flight payload. Each record states sold status, hammer price, event and run time, VIN, make, model, trim, gearbox, colour and image. The homepage is a bounded window into the latest event rather than the full historical archive; it still adds verified transaction prices, and the registry says so rather than implying complete archive coverage.' }),
  s({ id: 'rmsothebys', name: "RM Sotheby's", homepage: 'https://rmsothebys.com', category: 'auction-enthusiast', priceKinds: ['sold'],
    status: 'live', transport: 'fetch', access: 'feed',
    notes: 'Wired 2026-09-11 over the POST search API at /api/search/SearchLots. The response echoes its full option set, which is how the real filter names (Make, Model, FromYear, ToYear) were learned rather than guessed. 98,863 lots in total, 2,812 of them Porsche; one crawl returns 252 completed sales spanning 1952 to 2024 in three currencies. valueType decides everything: Sold is a sale, Asking is a private-sales listing, and Not Sold is a lot that failed its reserve whose value is the HIGH BID and must never be stored as a price anyone paid. No mileage and no VIN in the payload, which is the honest shape of the source. It also sells body panels titled exactly like the car they came off (a 911 front clip went for $549).' }),
  s({ id: 'goodingco', name: 'Gooding Christie\'s', homepage: 'https://www.goodingco.com', category: 'auction-enthusiast', priceKinds: ['ask', 'sold'],
    status: 'live', transport: 'fetch', access: 'feed',
    notes: 'Wired 2026-09-15 over the public GraphQL catalogue at cdn.goodingco.com/graphql used by the site itself. A 100-row page carries typed make, model, year, currency, salePrice, askingPrice, estimates, auction and image fields; the Porsche archive currently spans 934 records in ten requests. salePrice is kept as a completed sale and askingPrice as an ask. lowEstimate and highEstimate are preserved only in raw and can never become a transaction price. The catalogue also contains automobilia, so only records explicitly typed Cars survive.' }),
  ...
  s({ id: 'carandclassic', name: 'Car & Classic', homepage: 'https://www.carandclassic.com', status: 'live', transport: 'fetch', access: 'feed', category: 'auction-enthusiast', countries: ['GB'], priceKinds: ['ask', 'sold'],
    notes: 'Wired 2026-09-11 over the POST search API at /api/search/SearchLots that its own site calls. The response echoes its full option set, which is how the real filter names (Make, Model, FromYear, ToYear) were learned rather than guessed. 98,863 lots in total, 2,812 of them Porsche, and a single crawl returns 252 completed sales spanning 1952 to 2024 in three currencies. valueType is the field that matters: Sold is a sale, Asking is a private-sales listing, and Not Sold is a lot that failed its reserve whose value is the high bid, which must never be recorded as a price anyone paid. There is no mileage and no VIN in the payload, which is the honest shape of the source. It also sells body panels titled exactly like the car they came off (a 911 front clip went for $549), so the non-vehicle phrase list has to catch them.',
  }),
  ...
  s({ id: 'exoticcartrader', name: 'Exotic Car Trader', homepage: 'https://www.exoticcartrader.com', category: 'specialty',
      status: 'live', transport: 'fetch', access: 'html',
      notes: 'Wired 2026-09-14 through the server-rendered /htmx/cfs endpoint used by its own infinite scroll. Make and model are real endpoint parameters, offsets page in batches of 24, and only the active inventory fragment is read; the separate off-market feed is deliberately excluded. The source publishes asking price, mileage, image and listing URL but no VIN on its result cards.' }),
  ...
  s({ id: 'pistonheads', name: 'PistonHeads', homepage: 'https://www.pistonheads.com', category: 'marketplace', countries: ['GB'],
    status: 'live', transport: 'tls', access: 'payload',
    notes: 'Wired 2026-09-10. Inventory is in an embedded payload, not markup or schema.org, and the marker field is makeAnalyticsName. Adverts carry a full specificationData block, so mileage, gearbox, colour and body style arrive typed rather than parsed out of a headline. The payload also carries featured and sponsored adverts that the search parameter does not bound: a Porsche query returned a Toyota Yaris and an Audi R8 among the first six, so every advert is filtered on the make it states. Price-on-application and auction adverts are skipped rather than stored at zero.' }),
  ...
  s({ id: 'autoscout24', name: 'AutoScout24', homepage: 'https://www.autoscout24.com', category: 'marketplace',
      ...
      notes: 'Wired 2026-09-10. Inventory is in an embedded payload rather than markup or schema.org; the marker field is makeAnalyticsName. Adverts carry a full specificationData block, so mileage, gearbox, colour and body style arrive typed rather than parsed from a headline. The payload also carries featured and sponsored adverts that the search parameter does not bound: a Porsche query returned a Toyota Yaris and an Audi R8, so each advert is filtered on the make it states. Price-on-application and auction adverts are skipped rather than stored at zero.', status: 'live', transport: 'fetch'}),
  ...
  s({ id: 'blocket', name: 'Blocket', homepage: 'https://www.blocket.se', category: 'classified', countries: ['SE'], status: 'live', transport: 'fetch', access: 'jsonld', notes: 'Wired 2026-09-10. Inventory is in an embedded payload rather than markup or schema.org; the marker field is makeAnalyticsName. Adverts carry a full specificationData block, so mileage, gearbox, colour and body style arrive typed rather than parsed from a headline. The payload also carries featured and sponsored adverts that the search parameter does not bound: a Porsche query returned a Toyota Yaris and an Audi R8, so each advert is filtered on the make it states. Price-on-application and auction adverts are skipped rather than stored at zero.'}),
  ...
  s({ id: 'autotraderca', name: 'AutoTrader.ca', homepage: 'https://www.autotrader.ca', status: 'live', transport: 'tls', access: 'jsonld', category: 'marketplace', countries: ['CA'],
    notes: 'Wired 2026-09-10. Inventory is in an embedded payload rather than markup or schema.org; the marker field is makeAnalyticsName. Adverts carry a full specificationData block, so mileage, gearbox, colour and body style arrive typed rather than parsed from a headline. The payload also carries featured and sponsored adverts that the search parameter does not bound: a Porsche query returned a Toyota Yaris and an Audi R8, so each advert is filtered on the make it states. Price-on-application and auction adverts are skipped rather than stored at zero.',
  }),
  ...
  s({ id: 'goonet', name: 'Goo-net', homepage: 'https://www.goo-net.com', category: 'marketplace', countries: ['JP'],
    notes: 'Probed 2026-09-10: the brand page ships a 22-entry ItemList and none of it is a car for sale, only links to model categories. Needs the per-model listing page, not this one. Do not wire it against the brand index.',
  }),
  s({ id: 'carsensor', name: 'Carsensor', homepage: 'https://www.carsensor.net', category: 'marketplace', countries: ['JP'],
    notes: 'Probed 2026-09-10: 30 ItemList entries carrying a url and photos with no name and no price, so the shared JSON-LD adapter would run clean and store nothing. The price is in markup on the listing page.',
  }),
  ...
  s({ id: 'mercadolibreautos', name: 'MercadoLibre Autos', homepage: 'https://autos.mercadolibre.com.mx', status: 'live', transport: 'tls', access: 'jsonld', category: 'marketplace', countries: ['MX', 'AR', 'BR'],
    notes: 'Wired 2026-09-10. 48 priced Product nodes per search page, prices in ARS and never converted. Covers Argentina, Mexico and Brazil from one payload shape.',
  }),
