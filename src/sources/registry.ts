...
export const SOURCES: Source[] = [
  ...
  s({ id: 'autotrader', name: 'Autotrader', homepage: 'https://www.autotrader.com', category: 'marketplace',
      status: 'planned', transport: 'fetch',
      notes: 'Re-measured 2026-09-08: returns 200 to a plain fetch, so the block is gone, but the body is a 4KB shell with no listings and no JSON-LD. Access solved, extraction unsolved: find the endpoint its frontend calls.' }),
  ...
  s({ id: 'carvana', name: 'Carvana', homepage: 'https://www.carvana.com', category: 'retailer',
      status: 'via-aggregator', transport: 'fetch', reachableVia: { sourceId: 'autotempest', code: 'cv' },
      notes: 'Re-measured 2026-09-08: plain fetch 403, real Chromium 403, but a request with a Chrome TLS fingerprint returns 200 and 1.2MB. Reachable via transport/tls.ts; only the extractor remains.' }),
